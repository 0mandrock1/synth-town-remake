...
## Принципы ощущения (final lock)
- Shape-to-sound: визуальная форма здания обязана читать waveform без подсказки.
- Vehicle-to-envelope: тип транспорта влияет на динамику ноты (коротко/мягко/долго).
- First-loop onboarding: первый валидный мини-грув собирается игроком за 60-90 секунд.
- Zero dead actions: каждое действие имеет аудио- и визуальный фидбек.
...
### Shape-to-sound (обязательное соответствие)
- `sine`: округлая крыша и гладкий контур.
- `square`: прямые углы и плоская крыша.
- `triangle`: шпиль и явный треугольный силуэт.
- `sawtooth`: ступенчатый/зубчатый профиль.
- `pulse`: узкая башня с импульсной индикацией.
...
### Vehicle-to-envelope
| Тип транспорта | Attack | Decay | Velocity multiplier | Назначение |
|----------------|--------|-------|---------------------|------------|
| car            | 0ms    | 0.20s | 1.00                | Стандартный шаг секвенсора |
| bicycle        | 5ms    | 0.12s | 0.80                | Лёгкие короткие ноты |
| bus            | 10ms   | 0.45s | 1.20                | Более плотные/длинные акценты |

Формула громкости транспорта: `finalVelocity = baseVelocity * vehicleMultiplier`.
...
- Шаг 1: overlay с одной кнопкой `Start Audio`.
- Шаг 2: подсветить 3 действия: дорога -> здание -> play.
- Шаг 3: после первого trigger показать короткий hint "Город уже звучит".
- Не использовать длинные модальные туториалы.
